...
// Full game logic: questions, lives, timer,
// streaks, popups, solution, leaderboard
// =============================================

// ---- QUESTION BANK ----
// To add new questions, copy the format below and add to this array.
// type: 'mcq' or 'numerical'
// For numerical: answer is a number (or string for expressions like '-3')
// tolerance: how close the numerical answer needs to be (default 0)
...
const QUESTIONS = [
  ...
  },
  {
    ...
    solution: [
      ...
      { step: 2, text: "Set f'(x) = 0 to find stationary points.", formula: '3x² − 12x + 9 = 0  →  x² − 4x + 3 = 0  →  (x−1)(x−3) = 0' },
      { step: 3, text: 'So x = 1 or x = 3. Find the second derivative.', formula: "f''(x) = 6x − 12" },
      { step: 4, text: "At x = 1: f''(1) = 6(1) − 12 = −6 < 0, so x = 1 is a local maximum.", formula: 'Answer: x = 1' },
    ]
  },
  {
    id: 3,
  ...
  },
  {
    ...
    answer: 'A',
    solution: [
      { step: 1, text: 'The powers of i cycle with period 4: i¹=i, i²=−1, i³=−i, i⁴=1.', formula: 'i¹=i, i²=−1, i³=−i, i⁴=1, i⁵=i ...' },
      { step: 2, text: 'Divide the exponent by 4 and find the remainder.', formula: '25 ÷ 4 = 6 remainder 1' },
      { step: 3, text: 'A remainder of 1 means i²⁵ = i¹ = i.', formula: 'i²⁵ = i' },
      { step: 4, text: 'The answer is A: i.', formula: 'Answer: A' },
    ]
  },
  {
    id: 5,
    ...
    answer: 765,
    tolerance: 0,
    hint: 'Use the formula Sₙ = a(rⁿ − 1) / (r − 1) with a = 3, r = 2, n = 8.',
    solution: [
      { step: 1, text: 'Identify the values: a = 3, r = 2, n = 8.', formula: 'Sₙ = a(rⁿ − 1) / (r − 1)' },
      { step: 2, text: 'Substitute the values into the formula.', formula: 'S₈ = 3(2⁸ − 1) / (2 − 1)' },
      { step: 3, text: 'Calculate 2⁸ = 256.', formula: 'S₈ = 3(256 − 1) / 1 = 3 × 255' },
      { step: 4, text: 'Multiply to get the final answer.', formula: 'S₈ = 765' },
    ]
  },
];
...
// Pick today's question based on the date (cycles through questions)
...
function renderQuestion(q) {
  ...
  if (q.type === 'mcq') {
    ...
    const numWrap = document.getElementById('num-input-wrap');
...
function handleCorrect() {
  ...
  // Percentile (seeded pseudo-random so consistent for the day)
  const seed = new Date().toDateString().split('').reduce((a, c) => a + c.charCodeAt(0), 0);
  const percentile = 40 + (seed % 55); // 40–94%
...
function handleWrong() {
  ...
  // Animate life lost
  const lifeEl = document.getElementById(`life-${3 - lives + (3 - lives === 0 ? 0 : 0)}`);
  // Find the first remaining life element and mark it lost
  ...
  document.getElementById('attempts-left').textContent =
    remaining === 0 ? 'No lives left' : `${remaining} remaining`;
  ...
  if (lives <= 0) {
    ...
    // Show wrong popup with remaining lives
...
function updateYourLBRow(won, timeStr) {
  const youRows = document.querySelectorAll('.lb-you-row');
  youRows.forEach(row => {
...
document.querySelectorAll('.popup-overlay').forEach(overlay => {
  overlay.addEventListener('click', (e) => {
    if (e.target === overlay) {
      // Only close wrong popup on overlay click (not gameover or win)
      if (overlay.id === 'wrong-popup') closePopup('wrong-popup');
    }
  });
});
